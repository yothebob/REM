...
if (global.totalslider == true){
	// if slider is open
		...
		if check_parts_list(init.parts_dict){
			//if there are parts to draw (excluding touchup paint)
			draw_set_halign(fa_left);
			var o = 150;
			var key = variable_struct_get_names(init.parts_dict);
			var index_array = array_create(array_length(key),"");
		
			// loop through dictionary, each iteration check if part quanity is < 1, add to draw array
